...
def getCurrentStrTime():
    return time.strftime("_%Y-%m-%d %H'%M''%S'''")
...
def make_nene(dir_index, file_index):
    destination_csv = dir_name + dir_delimiter + nene_dir + str(dir_index) + dir_delimiter + nene_file + str(file_index) + getCurrentStrTime() + csv
...
#if not os.path.isdir() 파일이 있는지 확인하고                    Try 문보다
#    os.mkdir()         없으면 참(True)가 되기때문에 파일을 생성함.      더좋은 방법
