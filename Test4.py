# ?KEY=9fe040acbb464746855ec086eab4aa16
    ...
    try:
        ...
        if (jsonData['response']['header']['resultMsg'] == 'OK'):
            for i in (jsonData['response']['body']['items']['item']):
                jsonResult.append({'baseDate':i['baseDate'],'baseTime':i['baseTime'],
                                   'category':i["category"],'fcstDate':i['fcstDate'],
                                   'fcstTime':i['fcstTime'],'fcstValue':i['fcstValue'],
                                   'nx':i['nx'],'ny':i['ny']})
...
def Sunny_Day_simulation():
    ...
    if(retJson[0]['category']) == 'SKY':
        ...
        if retJson[0]['fcstValue'] <= 2:
            if g_humidifier == True and g_dehumidifier == True:
                ...
                print("***화창한 날씨입니다. 켜져있는 가습기의 작동을 정지하겠습니다.***")
                g_humidifier = not g_humidifier
                check_device_status()

            elif g_dehumidifier == True:
                print("***화창한 날씨입니다. 켜져있는 제습기의 작동을 정지하겠습니다.***")
                g_dehumidifier = not g_dehumidifier
                check_device_status()
